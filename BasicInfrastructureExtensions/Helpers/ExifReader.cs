...
namespace BasicInfrastructureExtensions.Helpers
{
    ...
    public sealed class ExifReader : IDisposable
    {
        ...
        private byte[] ReadBytes(ushort tiffOffset, int byteCount)
        {
            ...
            var originalOffset = _fileStream.Position;
        ...
        private void CreateTagIndex()
        {
            ...
            _fileStream.Position = _ifd1Offset + _tiffHeaderStart;
        ...
        private byte[] GetTagBytes(Dictionary<ushort, long> tagDictionary, ushort tagID, out ushort tiffDataType,
                                   out uint numberOfComponents)
        {
            // Get the tag's offset from the catalogue and do some basic error checks
            if (_fileStream == null || _reader == null || tagDictionary == null || !tagDictionary.ContainsKey(tagID))
            {
                tiffDataType = 0;
        ...
        private void CatalogueIfd(ref Dictionary<ushort, long> tagOffsets)
        {
            ...
            for (ushort currentEntry = 0; currentEntry < entryCount; currentEntry++)
            {
                ...
                tagOffsets[currentTagNumber] = _fileStream.Position - 2;
        ...
        public byte[] GetJpegThumbnailBytes()
        {
            ...
            int currentByte;
            var previousByte = -1;
            while ((currentByte = _fileStream.ReadByte()) != -1)
            {
            ...
            _fileStream.Position -= 2;

            var imageBytes = new byte[length];
            _fileStream.Read(imageBytes, 0, (int) length);
